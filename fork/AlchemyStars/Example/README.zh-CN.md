...
3. 使用路径右侧的文件夹按钮重新定位素材。动画主列表、动画层区域和模型部件列表的条目或空白处都支持右键导入；也可聚焦后按 `Shift+F10`。
4. 选择输出目录。需要保留原项目输出格式时，确认 `MP5Base` 为 `.cast`、`MP5Grip` 为 `.seanim`。
5. 使用“项目另存为”保存适配本机路径的副本，不要覆盖根目录中的标准示例。
6. 点击“保存动画”。炼金之星会让每个动画条目产生一个独立文件；CAST 输出包含共享模型数据和且仅包含当前条目的一个烘焙动画。导出时会自动按 ViewHands、Weapon、Attachment 规范化模型合并顺序，因此界面中的手动排列不会再造成 Maya 双骨架。
...
当前 Hawk 冲刺基准已在本机 Maya 2025 无界面模式验证：214 个关节、21 个可见网格、1284 条动画曲线、30 FPS、0–66 帧，并且场景中只有一个骨架根和一个 `j_gun`。两个 Additive 层已经烘焙进唯一动画，不会作为多余动画节点留下。
