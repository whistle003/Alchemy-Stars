...
6. 在“设置 → 输出”选择 `.cast`、`.fbx`、`.smd` 或 `.seanim`，再点击“保存动画”。“仅输出合并动画 CAST”会让 `.cast` 不含模型、网格、材质和蒙皮；“仅烘焙相关骨骼”只保留基础动画、姿势、动画层、IK 及间接受影响骨骼。两个选项会全局记忆并写入 `.aprj`。相关骨骼模式默认关闭；仅动画 CAST 导入已有绑定时，必须使用完全匹配且处于干净绑定姿势的骨架。SMD 因格式要求仍逐帧写出完整姿势。
