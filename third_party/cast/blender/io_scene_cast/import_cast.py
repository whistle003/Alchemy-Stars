...
def importMergeModel(self, selectedObj, skeletonObj, poses):
    ...
    while missingBones:
        for bone in [x for x in missingBones]:
            ...
            newBone.tail = 0, 1.0, 0  # Keep joint orientation stable at large float32 coordinates.
...
def importCast(self, context, path):
    ...
    for root in cast.Roots():
        for child in root.ChildrenOfType(Model):
            ...
            importedArmatures.extend(obj for obj in bpy.data.objects
                                     if obj not in beforeObjects and obj.type == 'ARMATURE')
        for child in root.ChildrenOfType(Animation):
            # Alchemy Stars packages one model and one baked animation together.
            # Bind that animation to the skeleton just imported, including in an empty scene.
            animationObject = importedArmatures[0] if len(importedArmatures) == 1 else selectedObject
            if animationObject is not None:
                bpy.context.view_layer.objects.active = animationObject
            importAnimationNode(self, child, path, animationObject)
