...
Choose `.cast`, `.fbx`, `.smd`, or `.seanim` under **Settings → Output**. **Animation-only CAST** removes model, mesh, material, and skin data from `.cast` output. **Bake relevant bones only** retains source-animation, pose, layer, IK, and indirectly changed bones. Both settings are remembered globally and in `.aprj` files.

Relevant-bone baking is off by default. Full-scene output is safe after the retained-curve checks; animation-only imports require the exact matching skeleton in a clean bind pose, so leave the option off for unrelated or modified rigs. SMD must still contain a complete pose per frame. FBX uses the official `fbxmaya` plug-in from the locally installed Maya (Maya 2025 is preferred and auto-detected).

The published `MayaPlugin` folder contains `cast.py` and `castplugin.py`. Add both to Maya's script/plugin path, load `castplugin.py`, and import the generated CAST through File → Import. A new scene does not require `Import Merge`; enable it only when intentionally merging into a skeleton that is already in the scene. FBX users can enable **Fill Timeline** on import. Verified CAST and FBX Hawk sprint results use 30 FPS, frames 0–66, 215 joints, 21 skinned meshes, one skeleton root, and separate `j_gun` and `j_gun__weapon` joints; the relevant-bone CAST retains 121 bones and is separately imported into Maya 2025. SMD is checked for 215 nodes, 67 frames, and weapon world-space motion.
